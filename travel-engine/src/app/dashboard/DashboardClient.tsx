...
export default function DashboardClient() {
  ...
  const allPoints = useMemo(
    () => getHeatmapPoints(startDate, endDate),
    [startDate, endDate],
  );
  const topDests = useMemo(
    () => getTopDestinations(startDate, endDate, 8),
    [startDate, endDate],
  );
  ...
  const filteredPoints = useMemo(
    () =>
      continent === "All"
        ? allPoints
        : allPoints.filter((p) => p.destination.continent === continent),
    [allPoints, continent],
  );
  ...
  const handlePlanTrip = useCallback(
    (dest?: HeatmapPoint) => {
      ...
      if (!target) return;
      router.push(
        `/plan?destination=${encodeURIComponent(target.destination.name)}&start=${startDate}&end=${endDate}&budget=${target.destination.avgBudget}`,
      );
    },
  ...
  return (
    <div className={styles.layout} aria-label="Travel density dashboard">
      <nav className={styles.nav} aria-label="WANDR navigation">
        ...
        <div
          className={styles.navPeriod}
          aria-label={`Vacation period: ${formatDate(startDate)} to ${formatDate(endDate)}`}
        >
      ...
      <div className={styles.body}>
        <aside className={styles.sidebar} aria-label="Top destinations sidebar">
          <div className={styles.statsRow}>
            <div
              ...
              aria-label={`${formatCount(totalTravelers)} total travelers`}
            >
              <span className={styles.statNum}>
                {formatCount(totalTravelers)}
              </span>
              <span className={styles.statLabel}>Travelers</span>
            </div>
            <div
              className={styles.statCard}
              aria-label={`${filteredPoints.length} destinations`}
            >
          ...
          <div
            className={styles.continentFilter}
            role="group"
          ...
          <ul
            ...
            role="list"
            aria-label="Top destinations by traveler count"
          >
            {topDests.map((p, i) => (
              <li key={p.destination.id}>
                ...
                >
                  <span className={styles.destRank} aria-hidden="true">
                    #{i + 1}
                  </span>
                  ...
                  <div className={styles.destMeta}>
                    <span className={styles.destCount}>
                      {formatCount(p.adjustedCount)}
                    </span>
                    <span className={styles.destBudget}>
                      ${p.destination.avgBudget.toLocaleString()}
                    </span>
                  </div>
                </button>
              </li>
          ...
          <button
            ...
            aria-label={
              selectedId
                ? `Plan trip to ${selectedPoint?.destination.name}`
                : "Select a destination to plan a trip"
            }
          >
            {selectedId
              ? `Plan Trip to ${selectedPoint?.destination.name.split(",")[0]}`
        ...
        <main className={styles.mapArea} aria-label="Interactive world heatmap">
          ...
          {selectedPoint && (
            <div
              className={styles.destTooltip}
              role="status"
              aria-live="polite"
            >
              <div className={styles.tooltipHeader}>
                <h2 className={styles.tooltipName}>
                  {selectedPoint.destination.name}
                </h2>
                <button
              ...
              <div className={styles.tooltipStats}>
                <div className={styles.tooltipStat}>
                  <span className={styles.tooltipStatNum}>
                    {selectedPoint.adjustedCount.toLocaleString()}
                  </span>
                  <span className={styles.tooltipStatLabel}>travelers</span>
                </div>
                <div className={styles.tooltipStat}>
                  <span className={styles.tooltipStatNum}>
                    ${selectedPoint.destination.avgBudget.toLocaleString()}
                  </span>
                  <span className={styles.tooltipStatLabel}>avg budget</span>
                </div>
                <div className={styles.tooltipStat}>
                  <span className={styles.tooltipStatNum}>
                    {selectedPoint.destination.continent}
                  </span>
