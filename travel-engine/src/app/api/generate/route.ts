...
export async function POST(req: NextRequest) {
  try {
    ...
    const { destination, startDate, endDate, budget, travelStyle, groupSize, interests, dietary, accessibility } = body;
    ...
    if (isNaN(Number(budget)) || Number(budget) < 100 || Number(budget) > 100000) {
      return NextResponse.json({ error: 'Budget out of valid range' }, { status: 400 });
    }
    ...
    const prompt = buildItineraryPrompt({
      ...
      accessibility: Array.isArray(accessibility) ? accessibility.slice(0, 5) : [],
    });
    ...
    if (!validation.success) {
      ...
      return NextResponse.json(
        { error: 'Invalid itinerary format from AI', details: validation.error.flatten() },
        { status: 422 }
      );
    }
